...
public class TestBase {
	...
	public void setUp() throws Exception {
		...
		if (System.getProperty("tunnelId") != null)
			capabilities.setCapability("tunnelId", System.getProperty("tunnelId"));
	...
	void connect() {
		String title = "event";
		reportiumClient.testStart("Junit Perfecto Connect Test", new TestContext("tunnel", "test")); // Starts the reportium test

		reportiumClient.stepStart("Add an event"); // Starts a reportium step
		driver.get("http://0.0.0.0:8090/");
		WebDriverWait wait = new WebDriverWait(driver, 5);
		wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//*[@placeholder='Title']")));
		driver.findElement(By.xpath("//*[@placeholder='Title']")).sendKeys(title);
		driver.findElement(By.xpath("//*[@placeholder='Detail']")).sendKeys("check out");
		driver.findElement(By.xpath("//*[@placeholder='Date']")).sendKeys("1999-08-08");
	...
	void failTest() {
		...
		reportiumClient.testStart("Junit Perfecto Connect Fail Test", new TestContext("fail", "tag3"));
		reportiumClient.stepStart("Verify test fails");
		driver.manage().timeouts().implicitlyWait(1, TimeUnit.SECONDS);
		driver.get("http://0.0.0.0:8090/");
		driver.findElement(By.xpath(
						"//*[contains(@placeholder,':test91823749283')]"))
				.isDisplayed();
		reportiumClient.stepEnd();
	}
